...
export async function getCommentsForPost(postId: number): Promise<Comment[]> {
    try {
        const comments = await apiFetch<Comment[]>(`/api/comments/post/${postId}`);
        return comments;
    } catch (error) {
 ...
 */
export async function getLikesForPost(postId: number): Promise<Like[]> {
    try {
        const likes = await apiFetch<Like[]>(`/api/likes/post/${postId}`);
        return likes;
    } catch (error) {
...
export async function likePost(input: {
    ...
    try {
        const created = await apiFetch<Like>('/api/likes', {
            method: 'POST',
            body: JSON.stringify(input),
        });
        return created;
 ...
 * DELETE /api/likes/{id}
 */
export async function unlike(id: number): Promise<boolean> {
    try {
        await apiFetch(`/api/likes/${id}`, {
            method: 'DELETE',
        });
        return true;
    } catch (error) {
...
export async function getCat(id: number): Promise<Cat | null> {
    try {
        const cat = await apiFetch<Cat>(`/api/cats/${id}`);
        return cat;
    } catch (error) {
...
export async function createCat(input: {
    ...
    try {
        const created = await apiFetch<Cat>('/api/cats', {
            method: 'POST',
            body: JSON.stringify(input),
        });
        return created;
...
export async function updateCat(
    ...
    try {
        const updated = await apiFetch<Cat>(`/api/cats/${id}`, {
            method: 'PUT',
            body: JSON.stringify(input),
        });
        return updated;
